...
import { IconCheck, IconChevron, IconGlobe, IconShare, IconUsers } from '../ds/icons';

function ServiceCard({ icon, title, body, points, onLearn }) {
  return (
    <Card variant="panel" interactive style={{ display: 'flex', flexDirection: 'column', padding: 30, borderRadius: 'var(--radius-2xl)' }}>
      ...
      <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 24, letterSpacing: '-0.02em', margin: '22px 0 10px' }}>{title}</h3>
      ...
      <div style={{ marginTop: 22 }}>
        <Button variant="text" iconRight={<IconChevron size={16} />} onClick={onLearn}>Learn more</Button>
      </div>
    </Card>
  );
}
    ...
    <>
      <section id="services" style={{ maxWidth: 1100, margin: '0 auto', padding: '80px 22px' }}>
        <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 50px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 'clamp(34px,4.6vw,52px)', letterSpacing: '-0.025em', lineHeight: 1.06, margin: 0 }}>
            Three ways to show up bigger.
          </h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
          <ServiceCard icon={<IconGlobe size={26} />} title="Web & Product"
            ...
            onLearn={() => goToPage('web-product-services')}
          />
          <ServiceCard icon={<IconShare size={26} />} title="Social & Content"
            ...
            onLearn={() => goToPage('social-content-services')}
          />
          <ServiceCard icon={<IconUsers size={26} />} title="Talent & Consultancy"
            ...
            onLearn={() => goToPage('talent-consultancy-services')}
          />
        </div>
      </section>
